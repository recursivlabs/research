  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        <div style={{ fontSize: 26, color: '#8a93a3', marginTop: 20, maxWidth: 920, lineHeight: 1.4 }}>
          {summary.length > 160 ? summary.slice(0, 157) + '…' : summary}
        </div>
        {stat && (
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 16, marginTop: 'auto' }}>
            <div style={{ fontSize: 72, fontWeight: 700, color: '#39e0c8' }}>{stat}</div>
            <div style={{ fontSize: 24, color: '#5b6472' }}>{statLabel}</div>
          </div>
        )}
      </div>
    ),
